...
export const verifyPaystackPayment = createAsyncThunk(
  ...
  async ({ reference, orderId }) => {
    const response = await axios.post(
      `${API_BASE_URL}/api/shop/order/verify-paystack`,
      {
        reference,
        orderId,
      },
...
export const verifyFlutterwavePayment = createAsyncThunk(
  "/order/verifyFlutterwavePayment",
  async ({ transaction_id, orderId }) => {
    const response = await axios.post(
      `${API_BASE_URL}/api/shop/order/verify-flutterwave`,
      {
        transaction_id,
        orderId,
      },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createNewOrder.fulfilled, (state, action) => {
        ...
        state.paystackAuthURL = action.payload.authorization_url || null;
        state.flutterwaveLink = action.payload.link || null;
